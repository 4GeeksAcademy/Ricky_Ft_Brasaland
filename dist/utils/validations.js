function validatePositivePrice(usd, cop, label) {
    const errors = [];
    if (usd <= 0) {
...
export function validateMenuItem(item) {
    ...
    errors.push(...validatePositivePrice(item.basePrice.USD, item.basePrice.COP, "Base price"));
    errors.push(...validatePositivePrice(item.ingredientCost.USD, item.ingredientCost.COP, "Ingredient cost"));
...
export function validateSaleTransaction(sale) {
    ...
    errors.push(...validatePositivePrice(sale.totalPrice.USD, sale.totalPrice.COP, "Total price"));
...
export function validateLocation(location) {
    ...
    errors.push(...validatePositivePrice(location.monthlyRentCost.USD, location.monthlyRentCost.COP, "Monthly rent cost"));
    errors.push(...validatePositivePrice(location.averageMonthlyUtilities.USD, location.averageMonthlyUtilities.COP, "Average monthly utilities"));
    return {
        valid: errors.length === 0,
        errors,
    };
